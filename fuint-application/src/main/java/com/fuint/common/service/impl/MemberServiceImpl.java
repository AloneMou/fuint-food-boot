...
public class MemberServiceImpl extends ServiceImpl<MtUserMapper, MtUser> implements MemberService {
     ...
     */
    @Override
    public MtUser queryMemberByOpenId(Integer merchantId, String openId, JSONObject userInfo, Integer type) throws BusinessCheckException {
        ...
        if (type == 0) {
            ...
            user = queryMemberByMpOpenId(merchantId, openId);
            if (user == null) {
                user = mtUserMapper.queryMemberByOpenId(merchantId, openId);
                user.setOpenId("");
...
//        }
        ...
        if (user == null) {
            ...
            if (type == 0) {
                ...
                mtUser.setMpOpenId(openId);
            }
            mtUser.setUnionid(unionid);
            MtUserGrade grade = userGradeService.getInitUserGrade(merchantId);
            if (grade != null) {
                mtUser.setGradeId(grade.getId() + "");
            }
            ...
            if (StringUtil.isNotBlank(unionid)) {
                ...
                user = mtUserMapper.queryMemberByMpOpenId(merchantId, openId).get(0);
            }
            // 开卡赠礼
