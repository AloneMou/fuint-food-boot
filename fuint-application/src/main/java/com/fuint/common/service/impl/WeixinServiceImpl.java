...
public class WeixinServiceImpl implements WeixinService {
    ...
    public JSONObject getWxProfile(Integer merchantId, String code) throws BusinessCheckException {
        ...
        try {
            ...
            if (!json.containsKey("errcode")) {
                ...
                logger.error("获取微信getWxProfile出错：code = {},msg={}", json.containsKey("errcode"), json.get("errmsg"));
            }
        } catch (Exception e) {
    ...
    public JSONObject getWxOpenId(Integer merchantId, String code) throws BusinessCheckException {
        ...
        try {
            ...
            if (!json.containsKey("errcode")) {
                ...
                logger.error("获取openId出错：code = {},msg={}", json.containsKey("errcode"), json.get("errmsg"));
            }
        } catch (Exception e) {
    ...
    public Boolean sendTemplateMessage(Integer merchantId, Integer userId, String toUserOpenId, String key, String page, Map<String, Object> params, Date sendTime) throws BusinessCheckException {
        ...
        MtSetting mtSetting = settingService.querySettingByName(merchantId, SettingTypeEnum.TEMP_MESSAGE.getKey(), key);
        if (mtSetting == null) {
            return false;
        }
        ...
        mtMessage.setMerchantId(merchantId);
        mtMessage.setUserId(userId);
        mtMessage.setType(MessageEnum.TEMP_MSG.getKey());
        mtMessage.setTitle(WxMessageEnum.getValue(key));
        mtMessage.setContent(WxMessageEnum.getValue(key));
        mtMessage.setIsRead(YesOrNoEnum.NO.getKey());
        mtMessage.setIsSend(YesOrNoEnum.NO.getKey());
    ...
    @Override
    public Boolean doSendTemplateMessage(Integer merchantId, String reqDataJsonStr) {
        try {
            String url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=" + getAccessToken(merchantId, false, true);
            String response = HttpRESTDataClient.requestPost(url, "application/json; charset=utf-8", reqDataJsonStr);
            logger.info("WeixinService doSendTemplateMessage response={}", response);
            JSONObject json = (JSONObject) JSONObject.parse(response);
            if (json.get("errcode").toString().equals("40001")) {
                getAccessToken(merchantId, true, true);
                logger.error("发送公众号模板消息出错error1：" + json.get("errcode").toString());
                return false;
            } else if (!json.get("errcode").toString().equals("0")) {
                logger.error("发送公众号模板消息出错error2：" + json.get("errcode").toString());
                return false;
            } else {
                return true;
            }
        } catch (Exception e) {
            logger.error("发送公众号模板消息出错：" + e.getMessage());
        }
        return true;
    }
