...
//        result.put("discountPrice", discountPrice);
    ...
    @Override
    public boolean sendTakeFoodRemind(OrderDto orderDto) throws BusinessCheckException {
        logger.info("orderService.updateOrder orderDto = {}", JsonUtil.toJSONString(orderDto));
        MtOrder mtOrder = mtOrderMapper.selectById(orderDto.getId());
        if (null == mtOrder || OrderStatusEnum.DELETED.getKey().equals(mtOrder.getStatus())) {
            throw new BusinessCheckException("该订单状态异常");
        }
        if (!PayStatusEnum.SUCCESS.getKey().equals(mtOrder.getPayStatus())) {
            throw new BusinessCheckException("该订单未支付");
        }
        if (!OrderModeEnum.ONESELF.getKey().equals(mtOrder.getOrderMode())) {
            throw new BusinessCheckException("该订单非自取订单");
        }
//        if (OrderStatusEnum.PAID.getKey().equals(mtOrder.getStatus())) {
//            throw new BusinessCheckException("该订单已支付");
//        }
        MtUser userInfo = memberService.queryMemberById(mtOrder.getUserId());
        MtStore store = storeService.queryStoreById(mtOrder.getStoreId());
//        MtMerchant merchantInfo = merchantService.queryMerchantById(mtOrder.getMerchantId());

        // 1分钟后发送小程序订阅消息
        Date nowTime = new Date();
        Date sendTime = new Date(nowTime.getTime() + 10000);
        Map<String, Object> params = new HashMap<>();
        String dateTime = DateUtil.formatDate(mtOrder.getPayTime(), "yyyy-MM-dd HH:mm");
        params.put("code", mtOrder.getVerifyCode());
        params.put("merchant", store.getName());
