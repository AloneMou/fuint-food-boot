...
public class PrinterServiceImpl extends ServiceImpl<MtPrinterMapper, MtPrinter> implements PrinterService {
    ...
    public Boolean printOrder(UserOrderDto orderInfo) throws Exception {
        ...
        for (MtPrinter mtPrinter : printers) {
            ...
            printContent.append("<C>").append("<B>").append(storeInfo.getName()).append("</B>")
                    .append("<BR></C>");
            if (orderInfo.getTableInfo() != null && StringUtils.isNotBlank(orderInfo.getTableInfo().getCode())) {
                printContent.append("<L>").append("<BOLD>桌码：").append(orderInfo.getTableInfo().getCode()).append("</BOLD><BR></L>");
            }

            printContent.append("<BR>");

            printContent
                    .append("品名").append(org.apache.commons.lang3.StringUtils.repeat(" ", 16))
            ...
            if (orderInfo.getGoods() != null && orderInfo.getGoods().size() > 0) {
                for (OrderGoodsDto goodsDto : orderInfo.getGoods()) {
                    printContent.append(NoteFormatter.formatPrintOrderItemForNewLine58(goodsDto.getName(), goodsDto.getNum(), Double.parseDouble(goodsDto.getPrice())));
                    if (goodsDto.getSpecList() != null) {
                        try {
                            List<String> specList = goodsDto.getSpecList().stream().map(GoodsSpecValueDto::getSpecValue).collect(Collectors.toList());
                            printContent.append("<L>")
                                    .append(org.apache.commons.lang3.StringUtils.repeat(" ", 2))
                                    .append(CollUtil.join(specList, ",")).append("<BR></L>");
                        } catch (Exception ex) {
                            logger.error("获取规格错误:{}", ex.getMessage());
                        }
                    }
            ...
            printContent.append("<BR>");
            printContent.append("<L>")
                    .append("订单编号：").append(orderInfo.getOrderSn()).append("<BR>")
                    ...
                    .append("订单备注：").append(orderInfo.getRemark()).append("<BR></L>");

            printContent.append("<C>")
                    .append("<QR>https://cyhy.aitecc.com/food</QR>")
                    .append("</C>");
